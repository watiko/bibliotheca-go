...
var (
	groupAllColumns            = []string{"group_id", "name", "updated_at", "created_at"}
	groupColumnsWithoutDefault = []string{"name", "updated_at", "created_at"}
...
// BookShelves retrieves all the book_shelf's BookShelves with an executor.
func (o *Group) BookShelves(mods ...qm.QueryMod) bookShelfQuery {
	...
	queryMods = append(queryMods,
		qm.Where("\"book_shelves\".\"group_id\"=?", o.GroupID),
	)
	...
	if len(queries.GetSelect(query.Query)) == 0 {
		queries.SetSelect(query.Query, []string{"\"book_shelves\".*"})
	}
...
func (o *Group) Users(mods ...qm.QueryMod) userQuery {
	...
	queryMods = append(queryMods,
		qm.InnerJoin("\"user_group_memberships\" on \"users\".\"user_id\" = \"user_group_memberships\".\"user_id\""),
		qm.Where("\"user_group_memberships\".\"group_id\"=?", o.GroupID),
	)
...
// LoadBookShelves allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (groupL) LoadBookShelves(ctx context.Context, e boil.ContextExecutor, singular bool, maybeGroup interface{}, mods queries.Applicator) error {
	var slice []*Group
	var object *Group
	...
	query := NewQuery(qm.From(`book_shelves`), qm.WhereIn(`book_shelves.group_id in ?`, args...))
	if mods != nil {
		mods.Apply(query)
	}
	...
	if err != nil {
		return errors.Wrap(err, "failed to eager load book_shelves")
	}

	var resultSlice []*BookShelf
	if err = queries.Bind(results, &resultSlice); err != nil {
		return errors.Wrap(err, "failed to bind eager loaded slice book_shelves")
	}

	if err = results.Close(); err != nil {
		return errors.Wrap(err, "failed to close results in eager load on book_shelves")
	}
	...
	if singular {
		object.R.BookShelves = resultSlice
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if local.GroupID == foreign.GroupID {
				local.R.BookShelves = append(local.R.BookShelves, foreign)
				if foreign.R == nil {
					foreign.R = &bookShelfR{}
				}
...
func (groupL) LoadUsers(ctx context.Context, e boil.ContextExecutor, singular bool, maybeGroup interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(
		...
		qm.From("\"users\""),
		qm.InnerJoin("\"user_group_memberships\" as \"a\" on \"users\".\"user_id\" = \"a\".\"user_id\""),
	...
	for results.Next() {
		...
		err = results.Scan(&one.UserID, &one.Name, &one.UpdatedAt, &one.CreatedAt, &one.Email, &localJoinCol)
		if err != nil {
...
// AddBookShelves adds the given related objects to the existing relationships
// of the group, optionally inserting them as new records.
// Appends related to o.R.BookShelves.
// Sets related.R.Group appropriately.
func (o *Group) AddBookShelves(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*BookShelf) error {
	var err error
	for _, rel := range related {
		if insert {
			...
			values := []interface{}{o.GroupID, rel.BookShelfID}
	...
	if o.R == nil {
		...
		o.R.BookShelves = append(o.R.BookShelves, related...)
	}
...
func (o *Group) AddUsers(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*User) error {
	...
	for _, rel := range related {
		query := "insert into \"user_group_memberships\" (\"group_id\", \"user_id\") values ($1, $2)"
		values := []interface{}{o.GroupID, rel.UserID}
...
func (o *Group) RemoveUsers(ctx context.Context, exec boil.ContextExecutor, related ...*User) error {
	var err error
	query := fmt.Sprintf(
		"delete from \"user_group_memberships\" where \"group_id\" = $1 and \"user_id\" in (%s)",
