...
var UserRels = struct {
	BorrowedByBooks string
	Groups          string
}{
	BorrowedByBooks: "BorrowedByBooks",
	Groups:          "Groups",
}

// userR is where relationships are stored.
type userR struct {
	BorrowedByBooks BookSlice
	Groups          GroupSlice
}
...
var (
	userAllColumns            = []string{"user_id", "name", "updated_at", "created_at", "email"}
	userColumnsWithoutDefault = []string{"name", "updated_at", "created_at", "email"}
...
// BorrowedByBooks retrieves all the book's Books with an executor via borrowed_by column.
func (o *User) BorrowedByBooks(mods ...qm.QueryMod) bookQuery {
	...
	queryMods = append(queryMods,
		qm.Where("\"books\".\"borrowed_by\"=?", o.UserID),
	)
...
func (o *User) Groups(mods ...qm.QueryMod) groupQuery {
	...
	queryMods = append(queryMods,
		qm.InnerJoin("\"user_group_memberships\" on \"groups\".\"group_id\" = \"user_group_memberships\".\"group_id\""),
		qm.Where("\"user_group_memberships\".\"user_id\"=?", o.UserID),
	)
...
// LoadBorrowedByBooks allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (userL) LoadBorrowedByBooks(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	var slice []*User
	var object *User
	...
	query := NewQuery(qm.From(`books`), qm.WhereIn(`books.borrowed_by in ?`, args...))
	if mods != nil {
		mods.Apply(query)
	}
	...
	if singular {
		object.R.BorrowedByBooks = resultSlice
		for _, foreign := range resultSlice {
			if foreign.R == nil {
				foreign.R = &bookR{}
			}
			foreign.R.BorrowedBy = object
		}
		return nil
	}
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if queries.Equal(local.UserID, foreign.borrowedUserId) {
				local.R.BorrowedByBooks = append(local.R.BorrowedByBooks, foreign)
				if foreign.R == nil {
					foreign.R = &bookR{}
				}
				foreign.R.BorrowedBy = local
				break
			}
		}
	}
...
func (userL) LoadGroups(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(
		...
		qm.From("\"groups\""),
		qm.InnerJoin("\"user_group_memberships\" as \"a\" on \"groups\".\"group_id\" = \"a\".\"group_id\""),
	...
	for results.Next() {
		...
		err = results.Scan(&one.GroupID, &one.Name, &one.UpdatedAt, &one.CreatedAt, &localJoinCol)
		if err != nil {
...
// AddBorrowedByBooks adds the given related objects to the existing relationships
// of the user, optionally inserting them as new records.
// Appends related to o.R.BorrowedByBooks.
// Sets related.R.BorrowedBy appropriately.
func (o *User) AddBorrowedByBooks(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Book) error {
	var err error
	for _, rel := range related {
		if insert {
			queries.Assign(&rel.borrowedUserId, o.UserID)
			...
			values := []interface{}{o.UserID, rel.BookID}
	...
	if o.R == nil {
		o.R = &userR{
			BorrowedByBooks: related,
		}
	} else {
		o.R.BorrowedByBooks = append(o.R.BorrowedByBooks, related...)
	}
...
// SetBorrowedByBooks removes all previously related items of the
// user replacing them completely with the passed
// in related items, optionally inserting them as new records.
// Sets o.R.BorrowedBy's BorrowedByBooks accordingly.
// Replaces o.R.BorrowedByBooks with related.
// Sets related.R.BorrowedBy's BorrowedByBooks accordingly.
func (o *User) SetBorrowedByBooks(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Book) error {
	query := "update \"books\" set \"borrowed_by\" = null where \"borrowed_by\" = $1"
	...
	if o.R != nil {
		for _, rel := range o.R.BorrowedByBooks {
			queries.SetScanner(&rel.borrowedUserId, nil)
			if rel.R == nil {
				continue
			}

			rel.R.BorrowedBy = nil
		}

		o.R.BorrowedByBooks = nil
	}
	return o.AddBorrowedByBooks(ctx, exec, insert, related...)
}

// RemoveBorrowedByBooks relationships from objects passed in.
// Removes related items from R.BorrowedByBooks (uses pointer comparison, removal does not keep order)
// Sets related.R.BorrowedBy.
func (o *User) RemoveBorrowedByBooks(ctx context.Context, exec boil.ContextExecutor, related ...*Book) error {
	var err error
	for _, rel := range related {
		queries.SetScanner(&rel.borrowedUserId, nil)
		if rel.R != nil {
		...
		if _, err = rel.Update(ctx, exec, boil.Whitelist("borrowed_by")); err != nil {
			return err
		}
	}
	if o.R == nil {
	...
	for _, rel := range related {
		for i, ri := range o.R.BorrowedByBooks {
			if rel != ri {
				continue
			}

			ln := len(o.R.BorrowedByBooks)
			if ln > 1 && i < ln-1 {
				o.R.BorrowedByBooks[i] = o.R.BorrowedByBooks[ln-1]
			}
			o.R.BorrowedByBooks = o.R.BorrowedByBooks[:ln-1]
			break
		}
	}
...
func (o *User) AddGroups(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Group) error {
	...
	for _, rel := range related {
		query := "insert into \"user_group_memberships\" (\"user_id\", \"group_id\") values ($1, $2)"
...
func (o *User) RemoveGroups(ctx context.Context, exec boil.ContextExecutor, related ...*Group) error {
	var err error
	query := fmt.Sprintf(
		"delete from \"user_group_memberships\" where \"user_id\" = $1 and \"group_id\" in (%s)",
		strmangle.Placeholders(dialect.UseIndexPlaceholders, len(related), 2, 1),
	)
	values := []interface{}{o.UserID}
	for _, rel := range related {
		values = append(values, rel.GroupID)
	}
