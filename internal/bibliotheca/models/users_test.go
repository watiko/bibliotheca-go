...
func testUserToManyBorrowedByBooks(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	queries.Assign(&b.borrowedUserId, a.UserID)
	queries.Assign(&c.borrowedUserId, a.UserID)
	...
	check, err := a.BorrowedByBooks().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	for _, v := range check {
		if queries.Equal(v.borrowedUserId, b.borrowedUserId) {
			bFound = true
		}
		if queries.Equal(v.borrowedUserId, c.borrowedUserId) {
			cFound = true
		}
	}
	...
	slice := UserSlice{&a}
	if err = a.L.LoadBorrowedByBooks(ctx, tx, false, (*[]*User)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.BorrowedByBooks); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.BorrowedByBooks = nil
	if err = a.L.LoadBorrowedByBooks(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.BorrowedByBooks); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testUserToManyGroups(t *testing.T) {
	...
	_, err = tx.Exec("insert into \"user_group_memberships\" (\"user_id\", \"group_id\") values ($1, $2)", a.UserID, b.GroupID)
	if err != nil {
		t.Fatal(err)
	}
	_, err = tx.Exec("insert into \"user_group_memberships\" (\"user_id\", \"group_id\") values ($1, $2)", a.UserID, c.GroupID)
	if err != nil {
		t.Fatal(err)
	}
...
func testUserToManyAddOpBorrowedByBooks(t *testing.T) {
	var err error
	...
	for i, x := range foreignersSplitByInsertion {
		err = a.AddBorrowedByBooks(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		if !queries.Equal(a.UserID, first.borrowedUserId) {
			t.Error("foreign key was wrong value", a.UserID, first.borrowedUserId)
		}
		if !queries.Equal(a.UserID, second.borrowedUserId) {
			t.Error("foreign key was wrong value", a.UserID, second.borrowedUserId)
		}

		if first.R.BorrowedBy != &a {
			t.Error("relationship was not added properly to the foreign slice")
		}
		if second.R.BorrowedBy != &a {
			t.Error("relationship was not added properly to the foreign slice")
		}

		if a.R.BorrowedByBooks[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.BorrowedByBooks[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.BorrowedByBooks().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
...
func testUserToManySetOpBorrowedByBooks(t *testing.T) {
	var err error
	...
	err = a.SetBorrowedByBooks(ctx, tx, false, &b, &c)
	if err != nil {
		t.Fatal(err)
	}
	...
	err = a.SetBorrowedByBooks(ctx, tx, true, &d, &e)
	if err != nil {
		t.Fatal(err)
	}
	...
	if !queries.IsValuerNil(b.borrowedUserId) {
		t.Error("want b's foreign key value to be nil")
	}
	if !queries.IsValuerNil(c.borrowedUserId) {
		t.Error("want c's foreign key value to be nil")
	}
	if !queries.Equal(a.UserID, d.borrowedUserId) {
		t.Error("foreign key was wrong value", a.UserID, d.borrowedUserId)
	}
	if !queries.Equal(a.UserID, e.borrowedUserId) {
		t.Error("foreign key was wrong value", a.UserID, e.borrowedUserId)
	}

	if b.R.BorrowedBy != nil {
		t.Error("relationship was not removed properly from the foreign struct")
	}
	if c.R.BorrowedBy != nil {
		t.Error("relationship was not removed properly from the foreign struct")
	}
	if d.R.BorrowedBy != &a {
	...
	if a.R.BorrowedByBooks[0] != &d {
		t.Error("relationship struct slice not set to correct value")
	}
	if a.R.BorrowedByBooks[1] != &e {
		t.Error("relationship struct slice not set to correct value")
	}
}

func testUserToManyRemoveOpBorrowedByBooks(t *testing.T) {
	var err error
	...
	err = a.AddBorrowedByBooks(ctx, tx, true, foreigners...)
	if err != nil {
		t.Fatal(err)
	}
	...
	err = a.RemoveBorrowedByBooks(ctx, tx, foreigners[:2]...)
	if err != nil {
		t.Fatal(err)
	}
	...
	if len(a.R.BorrowedByBooks) != 2 {
		t.Error("should have preserved two relationships")
	}

	// Removal doesn't do a stable deletion for performance so we have to flip the order
	if a.R.BorrowedByBooks[1] != &d {
		t.Error("relationship to d should have been preserved")
	}
	if a.R.BorrowedByBooks[0] != &e {
...
var (
	userDBTypes = map[string]string{`UserID`: `integer`, `Name`: `character varying`, `UpdatedAt`: `timestamp with time zone`, `CreatedAt`: `timestamp with time zone`, `Email`: `character varying`}
	_           = bytes.MinRead
)
