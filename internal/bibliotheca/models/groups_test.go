...
func testGroupToManyBookShelves(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err = randomize.Struct(seed, &b, bookShelfDBTypes, false, bookShelfColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	if err = randomize.Struct(seed, &c, bookShelfDBTypes, false, bookShelfColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	...
	check, err := a.BookShelves().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	slice := GroupSlice{&a}
	if err = a.L.LoadBookShelves(ctx, tx, false, (*[]*Group)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.BookShelves); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.BookShelves = nil
	if err = a.L.LoadBookShelves(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.BookShelves); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testGroupToManyUsers(t *testing.T) {
	...
	_, err = tx.Exec("insert into \"user_group_memberships\" (\"group_id\", \"user_id\") values ($1, $2)", a.GroupID, b.UserID)
	if err != nil {
		t.Fatal(err)
	}
	_, err = tx.Exec("insert into \"user_group_memberships\" (\"group_id\", \"user_id\") values ($1, $2)", a.GroupID, c.UserID)
	if err != nil {
		t.Fatal(err)
	}
...
func testGroupToManyAddOpBookShelves(t *testing.T) {
	var err error
	...
	foreigners := []*BookShelf{&b, &c, &d, &e}
	for _, x := range foreigners {
		if err = randomize.Struct(seed, x, bookShelfDBTypes, false, strmangle.SetComplement(bookShelfPrimaryKeyColumns, bookShelfColumnsWithoutDefault)...); err != nil {
			t.Fatal(err)
		}
	}
	...
	foreignersSplitByInsertion := [][]*BookShelf{
		{&b, &c},
		{&d, &e},
	}

	for i, x := range foreignersSplitByInsertion {
		err = a.AddBookShelves(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		if a.R.BookShelves[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.BookShelves[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.BookShelves().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
...
var (
	groupDBTypes = map[string]string{`GroupID`: `integer`, `Name`: `character varying`, `UpdatedAt`: `timestamp with time zone`, `CreatedAt`: `timestamp with time zone`}
	_            = bytes.MinRead
)
